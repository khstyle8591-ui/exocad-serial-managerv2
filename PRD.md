...
CREATE TABLE inbound_mails (
  ...
  classification   TEXT NOT NULL DEFAULT 'unclassified'
    CHECK(classification IN ('unclassified','stop_request','unrelated','error')),
...
| 채널 | 방향 | 설명 |
|---|---|---|
| `order:getPending` | R→M | pending_orders 목록 |
| `order:listAll` | R→M | 전체 주문 목록 |
...
```
대상: renewal_stop_requested=1 AND (status='active' OR status='expired') AND expiry_date <= today
...
POP3/IMAP 수신 → parseEmail → analyzeEmail:
  1. exclude_keywords 포함 → unclassified (전체 무시)
  2. dedicated_email 주소 → stop_request (무조건)
  3. product_keywords + action_keywords + (serial 형식) → stop_request
  4. product_keywords만 → unrelated (알림만)
  5. 해당 없음 → unclassified

stop_request → inbound_mails 저장 + pending_order 생성
unrelated → Slack 알림
```

> 현재 구현 기준: 수신 메일은 **즉시 캔슬하지 않음**. `stop_request`는 `inbound_mails`에 저장되고 운영자가 확인할 수 있도록 `pending_orders` 작업 항목을 만든다. `renewal_stop_requested` 플래그 자동 세팅은 현재 코드에 포함되어 있지 않다.
...
4. seedBuiltinTemplates() — 일본어 기본 3종
...
mail_classif_stop_request, mail_classif_unrelated, mail_classif_unclassified, mail_classif_error
...
| 3 | 중복 stop 메일 | `inbound_mails.message_id` UNIQUE + pending order `source_id` dedup |
...
- `src/server/routes/*` ✅ (REST API 라우터)
...
- Limbo fallback 실패 시 forced expired + critical Slack/SMTP 알림 코드 구현 필요
...
# 타입 체크
tsc --noEmit -p tsconfig.main.json   # main
tsc --noEmit                          # renderer

...
> **🔧 진행 상태 (2026-07-23 업데이트):** **F-1 ~ F-7 모두 ✅** 처리. F-7은 ESLint 최소 구성 도입 + 미사용 코드 10건 정리 + await-thenable 2건 조사(버그 아님 — 동기 함수의 no-op await 제거) 완료. **남은 것 하나:** ESLint `no-floating-promises` 26건(UI fire-and-forget 핸들러)은 케이스별 `void`/`.catch` 판단이 필요해 **다음 pass로 의도적으로 미룸**(비차단 warning이라 배포 무영향). 전체 검증(`verify` = 63 테스트 + DB 스키마 + 빌드) 통과. 브랜치 `feature/credit-system`.
...
| F-7 | ✅ 완료* | 🟡 LOW | 정적 린트 계층 부재 + 데드코드 (ESLint 도입·정리 완료, floating-promises 26건만 다음 pass) | 전역 | M |
